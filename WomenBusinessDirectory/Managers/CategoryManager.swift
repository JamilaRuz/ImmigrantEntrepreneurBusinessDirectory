...
class CategoryManager {
    ...
    func getCategoriesWithCompanyCount(
        ...
        let companyData = try snapshot.documents.compactMap { document -> (categoryIds: [String], city: String, ownershipTypes: [Company.OwnershipType]) in
            // Decode the full company to ensure data integrity
            let company = try document.data(as: Company.self)
            return (categoryIds: company.categoryIds, city: company.city, ownershipTypes: company.ownershipTypes)
        }
        ...
        return categories.map { category in
            let count = companyData.filter { companyInfo in
                ...
                // Apply ownership type filter if selected
                if !selectedOwnershipTypes.isEmpty {
                    let companyOwnershipTypes = Set(companyInfo.ownershipTypes)
                    if selectedOwnershipTypes.isDisjoint(with: companyOwnershipTypes) {
                        return false
                    }
                }
